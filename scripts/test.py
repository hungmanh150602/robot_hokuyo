            ...
            for i in range(detections.shape[2]):
                ...
                if confidence > 0.4:
                    ...
                    if class_id == 15: 
                        ...
                        # Lấy tâm người trực tiếp trên ảnh thô (méo)
                        center_mx = (mx1 + mx2) / 2.0
                        center_my = (my1 + my2) / 2.0
                        
                        # Đưa về hệ tọa độ gốc thấu kính (Optical Center)
                        x_ang = center_mx - cx_raw
                        y_ang = center_my - cy_raw
                        
                        # Tính bán kính góc r và góc phương vị
                        r = np.sqrt(x_ang**2 + y_ang**2)
                        
                        if r == 0:
                            angle_deg = 0.0
                        else:
                            # Khôi phục góc theta thực tế dựa trên mô hình thấu kính góc rộng
                            theta = r / fx_raw 
                            # Chiếu hướng lệch phương ngang (Yaw)
                            sin_phi = x_ang / r
                            angle_rad = theta * sin_phi
                            angle_deg = np.degrees(angle_rad)
                        
                        # --- TÍNH KHOẢNG CÁCH Z (Giữ nguyên phần nắn phẳng cục bộ) ---
                        ...
                        if np.any(valid_mask):
                            ...
                            if mean_disp > 0:
                                ...
                                print(f"[DATA CHUẨN] KhoangCach: {mean_Z:.2f}m | GocLechThuc: {angle_deg:.1f}°")
